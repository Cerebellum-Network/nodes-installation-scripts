...
function _stakerRewardsEras(instanceId, api) {
  return (0, _util2.memo)(instanceId, (eras, withActive) => (0, _xRxjs.combineLatest)([api.derive.staking._erasPoints(eras, withActive), api.derive.staking._erasPrefs(eras, withActive), api.derive.staking._erasRewards(eras, withActive)]));
}

function _stakerRewards(instanceId, api) {
  return (0, _util2.memo)(instanceId, (accountIds, eras, withActive) => (0, _xRxjs.combineLatest)([api.derive.staking.queryMulti(accountIds, {
    withLedger: true
    ...
    return api.derive.staking.queryMulti(allValidators, {
      withLedger: true
    }).pipe((0, _operators.map)(queriedVals => {
      return queries.map(({
        stakingLedger
      }, index) => {
...
function stakerRewards(instanceId, api) {
  return (0, _util2.memo)(instanceId, (accountId, withActive = false) => api.derive.staking.erasHistoric(withActive).pipe((0, _operators.switchMap)(eras => api.derive.staking._stakerRewards([accountId], eras, withActive)), (0, _operators.map)(([first]) => first)));
}

function stakerRewardsMultiEras(instanceId, api) {
  return (0, _util2.memo)(instanceId, (accountIds, eras) => accountIds.length && eras.length ? api.derive.staking._stakerRewards(accountIds, eras, false) : (0, _xRxjs.of)([]));
}
