...
function _ownExposures(instanceId, api) {
  ...
  return (0, _util.memo)(instanceId, (accountId, eras, _withActive) => eras.length ? api.queryMulti([...eras.map(era => [api.query.staking.erasStakersClipped, [era, accountId]]), ...eras.map(era => [api.query.staking.erasStakers, [era, accountId]])]).pipe((0, _operators.map)(all => eras.map((era, index) => ({
    clipped: all[index],
    era,
...
function ownExposure(instanceId, api) {
  return (0, _util.memo)(instanceId, (accountId, era) => api.derive.staking._ownExposures(accountId, [era], true).pipe((0, _operators.map)(([first]) => first)));
}
